...
A visualization of the differences between weighted and unweighted classifcation is provided below. Note this example is carried out with a provided dummy data as the example Rattini tribe dataset is hyperdimensional and representation of it in 2D can causing seemingly distant nearest neighbours to be nearest neighbours, because they are closer on other dimensions.
...
We have kept *k* set to 10 so far, but with a change in *k* the results might (and usually do!) change. Therefore we need to optimise *k* for classification in any given dataset. KnnDist provides a specific tool for this in the form of KnnDistCVStepwise and KnnDistCVStepwisePar (the parallel processing equivelant).

The simplest way to assess for optimising *k* is to start at *k*=1 and then increase *k* in a stepwise manner to a given number (e.g. maybe the smallest sample size in the dataset) and assess the classification accuracy at each step.

This can be done with the function KnnDistCVStepwise as follows (note in the first instance this will be done with Equal set to FALSE so the results will be with unequal sample size as demonstration, but the default is to resample to equal sample size 100 times):
...
``` {r k optimisation with stepwise correct cross validation with resampling}
...
Finally, if the stepwise *k* optimisation has already been carried out and you wish to plot the results again, then the function PlotStepwise can be used as follows (note that additional stepwise results can be added to the plot using the Add argument):

``` {r Plotting stepwise k results}
PlotStepwise(StepwiseResultsMat = EqualKOpt$Unweighted, 
             Percentiles = c(0.05,.95), 
             PlotCol = 'lightblue')

PlotStepwise(StepwiseResultsMat = EqualKOpt$Weighted.Results, 
             Percentiles = c(0.05,.95),
             PlotCol = 'darkblue',
             Add = TRUE)
